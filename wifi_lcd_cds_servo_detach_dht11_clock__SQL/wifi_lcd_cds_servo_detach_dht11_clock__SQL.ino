...
char sendId[10] = "HMG_ARD";
char sendBuf[CMD_SIZE];
char lcdLine1[17] = "Smart IoT By HMG";
...
void loop() {
  ...
  if (timerIsrFlag) //1초에 한번씩 실행
  {
    ...
    //  센서 데이터를 주기적으로 전송
    if (sensorTime != 0 && !(secCount % sensorTime )) //  주기가 0이 아니고 secCount가 주기의 배수일때 
    {
      //sprintf(sendBuf, "[%s]SENSOR@%d@%d@%d@%d@%d@%s@%s@%s@%s\r\n", getSensorId, (int)infra, (int)smoke, (int)temp, (int)humi, (int)gps, cam, buzz, led, motor);
            char 
            char tempStr[5];
            char humiStr[5];
            char 
            ...
            sprintf(sendBuf,"[%s]SENSOR@%d@%s@%s\r\n",getSensorId,infra,smoke, tempStr,humiStr, gps, cam, buzz, led, motor);
      
      client.write(sendBuf, strlen(sendBuf)); //  sendBuf에 저장된 데이터를 클라이언틀 전송
      client.flush(); //  즉시 전송 
    }
...
/*  else if (!strncmp(pArray[1], "GETSENSOR", 9)) { // 9개의 문자가 일치하면
    if (pArray[2] != NULL) { // 3번째 인자가 NULL이 아니면
      sensorTime = atoi(pArray[2]); // 주기 = 3번째인자 문자열(정수로 변환)
      strcpy(getSensorId, pArray[0]); // getSensorId에 아이디 복사
      return; // 함수 종료 
    } else {  // 앞선 조건 문이 거짓이면 
      sensorTime = 0; 
...
*/
    ...
    if (pArray[2] != NULL) {
      ...
      sensorTime = 0;
      sprintf(sendBuf, "[%s]%s@%d@%d@%d@%d@%d@%s@%s@%s@%s\n", pArray[0], pArray[1], (int)infra, (int)smoke, (int)temp, (int)humi, (int)gps, cam, buzz, led, motor);
    } 
  }
